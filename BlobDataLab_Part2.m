...
%and "indlat")
[lonmin, indlon]= min(abs(woa.lon-(-144.4)));

[latmin, indlat] = min(abs(woa.lat - 50.3));
...
[indepthmin, inddepth] = min(abs(woa.depth - 19.8));
...
time_s= ncread(satellite_filename, "time");

netcdf.getAtt(ncid_satellite, netcdf.inqVarID(ncid_satellite, 'time'), 'units') % seconds since 1970-01-01T00:00:00Z

satellite_time =datenum('1970-01-01 00:00:00');
...
[latmins, indlats] = min(abs(lats - 50.3));
